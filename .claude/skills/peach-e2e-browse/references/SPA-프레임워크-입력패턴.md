...
curl -s http://127.0.0.1:${E2E_CDP_PORT:-9333}/json | node -e \
...
const res = await fetch('http://127.0.0.1:${E2E_CDP_PORT:-9333}/json');
...
## 5. tab list 후 작업 탭 변경 감지

### 원인

사용자가 작업 탭을 응답하는 사이(수 초~수십 초) Chrome에서 탭이 닫히거나 agent-browser daemon이 재시작되면 기존 참조가 무효가 될 수 있다.

### 해결 — tabId/label 전환 직후 title 재확인 필수

```bash
# 1. 사용자가 "t2 탭" 또는 label을 응답한 뒤
...
**규칙**: `agent-browser tab <tabId|label>` 직후 반드시 `eval "document.title + ' | ' + location.href"`로 탭 상태를 검증한다.
예상과 다르면 재확인.
