...
`agent-browser connect ${E2E_CDP_PORT:-9333}` 실행 시 상주 daemon이 생성된다.
또한 AI 코딩 IDE의 playwright driver(`run-driver`)나 `playwright-cli` 상주 daemon(`cliDaemon.js`)처럼 같은 CDP 포트에 붙은 외부 클라이언트도 같은 증상을 만들 수 있다.
이 클라이언트들은 Playwright 계열 CDP 클라이언트로, 브라우저의 native `alert/confirm/prompt`를
...
lsof -iTCP:${E2E_CDP_PORT:-9333} -sTCP:ESTABLISHED -n -P

# 2. PID별 명령 확인 후 정확한 외부 CDP 클라이언트만 종료
ps -p <확인한_PID> -o command=
kill -TERM <확인한_CDP_클라이언트_PID>
sleep 2
kill -0 <같은_PID> 2>/dev/null && kill -KILL <같은_PID>

# 3. 종료 확인 (출력 없으면 정상)
lsof -iTCP:${E2E_CDP_PORT:-9333} -sTCP:ESTABLISHED -n -P
```

> 포트에 연결된 Node 전체를 종료하지 않는다. `daemon.js`·`cliDaemon.js`·`run-driver` 등 명령을 확인하고 TERM을 먼저 보낸다. KILL은 같은 PID가 남았을 때만 사용한다.

### 브라우저 재시작이 필요한 경우

- **거의 없다.** daemon만 종료하면 브라우저는 정상 동작한다.
- CDP 포트 자체가 응답하지 않을 때 재시작: `curl -s http://127.0.0.1:${E2E_CDP_PORT:-9333}/json/version` 실패 시
- daemon 종료 후에도 dialog가 자동으로 닫히면 → 다른 CDP 클라이언트가 붙어 있는지 `lsof` 재확인
- 외부 클라이언트가 없는데 `PLAYWRIGHT_ATTACH_TIMEOUT`이 반복되거나 `E2E_BROWSER_STATE=stale`이면 탭 영향을 보고하고 사용자 승인 후 재시작
...
- 명령을 확인하지 않은 PID 종료·일괄 `kill -9` **금지** → 정확한 CDP 클라이언트에 TERM 우선, 생존 시에만 KILL
